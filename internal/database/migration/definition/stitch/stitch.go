	...
	for _, rev := range revs {
		...
		for i, newDefinition := range revDefinitions.All() {
			...
			if !isSquashedMigration && !compareDefinitions(newDefinition, existingDefinition) {
				// Rewritten for idempotency in #31656
				if newDefinition.ID == 1528395955 ||
					newDefinition.ID == 1528395959 ||
					newDefinition.ID == 1528395965 ||
					newDefinition.ID == 1528395970 ||
					newDefinition.ID == 1528395971 ||
					newDefinition.ID == 1644515056 ||
					newDefinition.ID == 1645554732 {
					// fmt.Printf("WE KNOW, IS OK...\n")
				} else {
					fmt.Printf("> %v\n> %v\n", newDefinition.UpQuery.Query(sqlf.PostgresBindVar), existingDefinition.UpQuery.Query(sqlf.PostgresBindVar))
